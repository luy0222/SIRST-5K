...
image_Path = 'Demos/input/NUDT-SIRST/masks'
save_Path_1 = 'Demos/output/3_masks'
...
with open('coordinates.txt', 'r') as file:
    file = file.readlines()
    imgNames_l = os.listdir(image_Path)
                ...
                for j in str_list:
                    for k in range(4):
                            rot_patch_3(os.path.join(save_Path_1),
                                        os.path.join(image_Path, "{}.png".format(imgNames_s[imgNames_s.index(img)])),
                                        str_list[str_list.index(j)], k,str_list.index(j))
                            print('save image', imgNames_s[imgNames_s.index(img)])
