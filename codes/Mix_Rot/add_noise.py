...
imagePath = 'Demos/images'
noisePath = 'Demos/noise'

savePath_1 = 'Demos/mix/0.1'
savePath_2 = 'Demos/mix/0.2'
savePath_3 = 'Demos/mix/0.3'
savePath_4 = 'Demos/mix/0.4'
savePath_5 = 'Demos/mix/0.5'
...
create_folder(savePath_1)
create_folder(savePath_2)
create_folder(savePath_3)
create_folder(savePath_4)
create_folder(savePath_5)
...
for i in imgNames_l:
    for m in M:
        ...
        if M.index(m) == 0:
            cv2.imwrite(os.path.join(savePath_1, "{}".format(i)), tensor2img(infuse_image))
        if M.index(m) == 1:
            cv2.imwrite(os.path.join(savePath_2, "{}".format(i)), tensor2img(infuse_image))
        if M.index(m) == 2:
            cv2.imwrite(os.path.join(savePath_3, "{}".format(i)), tensor2img(infuse_image))
        if M.index(m) == 3:
            cv2.imwrite(os.path.join(savePath_4, "{}".format(i)), tensor2img(infuse_image))
        if M.index(m) == 4:
            cv2.imwrite(os.path.join(savePath_5, "{}".format(i)), tensor2img(infuse_image))
        print(i, m, 'done')
